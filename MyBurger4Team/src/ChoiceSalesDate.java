...
public class ChoiceSalesDate extends JFrame implements ActionListener {
	...
	public ChoiceSalesDate() {
		...
		pnlSouth.setBorder(BorderFactory.createEmptyBorder(10, 0, 20, 0));// 상 좌
																			// 하
																			// 우
																			// 여백
	...
	public void actionPerformed(ActionEvent e) {
		// 서브창(JDialog) 열기
		// ManagerTotalSales(ChoiceSalesDate csd, String title, boolean modal,
		// String year, String month, String day)
		try {
			// 매출 파일이 있으면 서브창 열고 아니면 알림만 뜨게!
			String date = yearCb.getSelectedItem().toString() + monthCb.getSelectedItem().toString()
					+ dayCb.getSelectedItem().toString();
			File f = new File(date + "sales.txt");

			if (f.exists()) {// 매출 파일이 존재하는 경우
				new ManagerTotalSales(this, "날짜별매출", true, yearCb.getSelectedItem().toString(),
						monthCb.getSelectedItem().toString(), dayCb.getSelectedItem().toString());
			} else {
